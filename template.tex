...
\begin{abstract}
    ...
    En effet, dans de nombreuses pratiques de création, musicales ou scénographiques, un travail sur des paramètres à plusieurs dimensions apparait nécessaires.
    ...
    Il offre des moyens d'écriture très généraux ainsi que la possibilité de spécialiser et d'optimiser les cas particuliers lorsque cela s'avère nécessaire.
...
C'est au prix de performances non optimales, qui peuvent néanmoins être optimisées par la suite.
...
Une version complète de cet état de l'art est présentée dans le compte-rendu du projet ANR OSSIA.
...
De même, une présentation de l'état actuel de l'écriture spatiale en musique est donnée dans~\cite{fober_les_2015}. Notamment, la question de la notation dans le cadre de partitions impliquant des éléments spatiaux est abordé.
...
Une autre question est l'association entre l'aspect graphique et le résultat. Ainsi, des outils tels que HoloEdit et HoloSpat permettent de travailler avec des trajectoires, mais sont orientés vers un travail sur de l'audio. 
...
Blender a déjà été utilisé à d'autres occasions pour de la spatialisation de son\cite{olaiz20093d}.
...
très riches pour dispoer et animer ces sources dans le temps.
...
commandes telles que \verb|dx 5| pour indique une vitesse de $\SI{5}{\centi\meter\per\second}$.
...
Sont prévus par la suite des implémentations spécifiques au moins pour les droites, segments, plans et demi-plans, polygones, cercles, sphères, et tores.
...
Il est important de noter que la seule équation $y_v = sin(u)$ ne suffit pas. 
...
De la même manière, une zone qui ne contraindrait aucunes des dimensions correspond à l'intégralité de l'espace.
...
Le résultat des calculs est ensuite exposé dans l'arbre i-score, puis être réutilisé
...
Néanmoins, à terme, un modèle de calcul permettant du chaînage dans un seul tick sera nécessaire.
...
\begin{figure}
    ...
    \caption{Une spline 3D dans i-score. On affiche actuellement la courbe dans le panneau inférieur en raison d'une limitation de l'implémentation du panneau central.}
    \label{fig.spline}
\end{figure}
...
\begin{itemize}
    ...
    \item Les automations à deux ou trois dimension : nous utilisons ici la bibliothèque \textit{vtk} pour afficher et éditer une spline à l'écran, à l'aide de l'objet~: ~\\ \verb|vtkSplineWidget|. 
...
\begin{itemize}
    ...
    Ainsi, un méchanisme de requête - réponse multi-threadé a été implémenté.
...
D'autres méthodes sont à étudier, par exemple en utilisant des méthodes de triangulation qui ont été évoquées plus tôt.
...
Nous avons présenté un extrait d'état de l'art des outils d'écriture spatiale 
...
financé par le projet ANR-12-COORD-0024.

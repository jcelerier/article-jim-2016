...
\begin{figure}[h]
	...
	\caption{Un scénario i-score de type Sonopluie : l'écran contient une boucle dont le motif consiste en deux processus simultanés, un processus Scénario qui gère la logique, et un processus Espace qui gère les objets spatiaux (ici trois disques).}
	\label{fig.sonopluie}
\end{figure}
...
Dans l'attente d'un modèle de calcul plus complet et générique, cela peut être fait dans le processus qui gère des trajectoires.
\section{Modèle}
...
\begin{itemize}
    \item Il est difficile de rajouter de nouveaux types de calculs, car il faut modifier tous les types pour rajouter le calcul (par exemple un calcul d'aire pour une zone).
...
\begin{itemize}
    \item Dans le cas du processus Espace, la création se fait en deux phases : 
    \begin{itemize}
        \item Spécification des zones et association des paramètres de zones à des paramètres de l'arbre. Cela est fait depuis une fenêtre séparée qui permet de donner des formules sous forme textuelle.
        \item Translation, mise à l'échelle et rotation via des outils comme on peut en trouver dans les logiciels d'édition et de modélisation standards. 
        Cela est fait directement dans l'interface de i-score.
    \end{itemize}
    ...
    Là encore, déplacement et rotations sont possibles.
\end{itemize}

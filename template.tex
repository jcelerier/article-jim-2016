...
Les pratiques multimédia incluent par définition des composantes spatiales.
...
trajectoires pour des sons, dans un cadre musical, soient orientées vers la création de jeu vidéo 
...
pour ensuite pouvoir les appliquer aux cas particuliers de la création musicale notamment dans un cadre interactif.
...
Ce modèle, intégré au séquenceur i-score, sera appliqué à deux études de cas : une chorégraphie de robots, 
et une application de réalité augmentée audio.

Nous discuterons ensuite des évolutions possibles qui sont liées à un typage plus fort des données
avec lesquelles on travaille : notamment les modes d'édition et de représentation qui peuvent être appliqués,
ainsi que de la quesiton de la relation au temps d'objets spatiaux en deux dimensions ou plus.


\subsection{Existant}
% TODO un état de l'art plus complet est présent dans le rapport du projet OSSIA.
...
La spatialisation est réalisée par la bibliothèque OpenAL % TODO Cite

Il manque un outil simple d'écriture pour disposer les zones dans l'espace.
De plus, on désire offrir de l'interaction et de l'évolutivité dans les scénarios : 
par exemple, après s'être approché d'un point donné, on voudrait pouvoir désactiver
des points précédents et activer des points suivants. 
...
Ce projet consiste en la réalisation d'une chorégraphie de robots musicaux.
On dispose d'une flotte de robots Metabot % TODO cite
Ces robots se contrôlent en vitesse : on écrit sur le port série des 
...
Les drones portent des hauts-parleurs tandis que les robotless font du rythme en tapant des pieds.
...
Nous offrons dans le logiciel les deux possibilités, mais l'affiHello,chage dans le premier cas n'est pas implémenté (le temps sera ceci dit pris en compte lors de l'exécution).
...
Cependant, cela ne s'est pas révél adapté : en effet la bibliothèque n'est pas thread-safe ce qui empêche de réaliser des calculs déportés sur un autre thread.
...
D'autres alternatives sont Symbolic C++, Sage, GNU Octave.
...
En effet, la valeur des paramètres de zones serait indéterminée si elle venait à être mise à jour 
par le thread d'exécution, ainsi que par le thread de réception de messages.
